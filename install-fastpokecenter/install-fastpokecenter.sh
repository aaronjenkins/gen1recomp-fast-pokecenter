#!/usr/bin/env bash
# Install (or refresh) the fast_pokecenter mod into the local gen1recomp data dir,
# backing up the Crystal save first.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LOG="$SCRIPT_DIR/install-fastpokecenter.log"
exec > >(tee -a "$LOG") 2>&1
printf '\n=== install-fastpokecenter run %s ===\n' "$(date -Iseconds)"

REPO="$(cd "$SCRIPT_DIR/.." && pwd)"
SRC="$REPO/code/fast_pokecenter"
...
mkdir -p "$MODS"
rm -rf "$MODS/fast_pokecenter"
cp -R "$SRC" "$MODS/fast_pokecenter"
echo "installed -> $MODS/fast_pokecenter"
ls -la "$MODS/fast_pokecenter"
